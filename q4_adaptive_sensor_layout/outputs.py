...
Validation weights: pd `{context["config"].validation_weight_pd}`, low_snr `{context["config"].validation_weight_low_snr}`, min_pd `{context["config"].validation_weight_min_pd}`, fa_excess `{context["config"].validation_weight_fa_excess}`
Robustness scenario: response_gain_jitter `{context["config"].response_gain_jitter}`, response_phase_jitter `{context["config"].response_phase_jitter}`, response_jitter_mode `{context["config"].response_jitter_mode}`, noise_correlation `{context["config"].noise_correlation}`
"""
...
def write_runtime(path: Path, context: dict) -> None:
    ...
    lines = [
        ...
        f"response_jitter_mode={cfg.response_jitter_mode}",
        f"noise_correlation={cfg.noise_correlation}",
        f"validation_weight_pd={cfg.validation_weight_pd}",
        f"validation_weight_low_snr={cfg.validation_weight_low_snr}",
        f"validation_weight_min_pd={cfg.validation_weight_min_pd}",
        f"validation_weight_fa_excess={cfg.validation_weight_fa_excess}",
    ]
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
Monte Carlo 验证排序权重为：`pd={context["config"].validation_weight_pd}`，`low_snr={context["config"].validation_weight_low_snr}`，`min_pd={context["config"].validation_weight_min_pd}`，`fa_excess={context["config"].validation_weight_fa_excess}`。
...
- response jitter mode: `{context["config"].response_jitter_mode}`
