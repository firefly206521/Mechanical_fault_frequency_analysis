...
远程拉取的版本（`c618b43`）已修复本报告之前指出的 **全部 10 个问题**：

| ID | 问题 | 修复方式 |
|----|------|----------|
| P1-001 | `estimated_full_200` 硬编码 | cli.py:345-359 改为使用 CLI 参数 + `projected_stage` 函数 |
| P1-002 | `read_csv_rows` 全量读取 | cli.py:262-270 使用 `extend` 就地维护行列表 |
| P2-003 | `close_pair_resolver` 对称约束 | core.py:597-716 完全重写为 peel/rescan/refine 流程 |
| P2-004 | `empirical_limit` 回落敏感性 | experiments.py:441 增加 `<=1` 容差 |
| P2-005 | 种子硬编码 | experiments.py:41 改为 `from . import SEED` |
| P2-006 | `_multi_sse` 重复构建设计矩阵 | core.py:206-220 改为只更新被修改的 sin/cos 列 |
| P3-007 | golden-section 过度收敛 | core.py:217/463/469/480/483 从 28 降至 18 |
| P3-008 | `_match_frequencies` 排序配对 | experiments.py:214-226 新增 `_nearest_mae` 贪心最近邻 |
| P3-009 | BIC 公式重复 | core.py:89-90 提取 `compute_bic` 共享函数 |
| P3-010 | `q3_timing.csv` 缺失 | cli.py:390-403 新增结构化时间输出 |

## 进一步优化建议（当前版本 `c618b43`）

### S1 - `append_csv_rows` 每次 batch flush 做冗余头部校验

- **文件**：`q3_multisource_separation/outputs.py`
- **行号**：26-44
- **严重度**：P2
- **类型**：性能
- **问题**：`append_csv_rows` 在每次 batch flush 时都重新从 CSV 文件读取第一行头部，与当前 fieldnames 做集合比较。同一 stage 内 fieldnames 不会变化，此校验只需在第一次 flush 或文件首次创建时执行一次。
...
if exists:
    ...
    if set(existing_header) != set(fieldnames):  # 校验
        raise ...
    fieldnames = existing_header  # 弃用当前 fieldnames
```

- **影响**：200 次仿真 5 SNR × 10 batch = 50 次；近频 4800 行 ÷ 20 = 240 次；纯噪声 500 ÷ 20 = 25 次；极端情况 900 ÷ 20 = 45 次；合计约 **360 次额外文件 open/close**。虽然单次成本低（< 0.5 ms），但累计浪费约 150-200 ms，且代码复杂度不必要。
- **建议修复**：在 `_run_tasks` 或 `append_csv_rows` 中缓存头部校验状态：
```python
def append_csv_rows(path, rows, _first_append=set()):
    key = str(path.resolve())
    exists = path.exists() and path.stat().st_size > 0
    if exists and key not in _first_append:
        # 做头部校验
        ...
        _first_append.add(key)
        fieldnames = existing_header
    ...
```
或更简洁地让调用方在 stage 开始时显式 `ensure_csv_schema(path, fieldnames)`，后续 `append_csv_rows` 跳过校验。

---

### S2 - `core.py` 达 758 行，建议按功能拆分

- **文件**：`q3_multisource_separation/core.py`
- **行号**：全文件
- **严重度**：P3
- **类型**：可维护性
- **问题**：单个文件包含 7+ 个功能领域（GLRT/设计矩阵/顺序检测/基带处理/近频分解/MUSIC/Hessian不确定性），模块间耦合松散但分隔不清晰。随 `compare_optimization.py`、`failure_stability.py` 等新文件增加，`core.py` 仍在扩张。
- **建议拆分**：
```
core.py                  → 顶层 API 和兼容导入
fitting.py               → 设计矩阵、multi_harmonic_fit、_multi_sse、refine_joint_frequencies
glrt.py                  → GLRT 统计量、门限、scan、detect_multitone
close_pair.py            → 基带处理、close_pair_resolver、music_close_pair、conditional_glrt_threshold
diagnostics.py           → design_diagnostics、segment_stability、_frequency_uncertainty
```
- **影响**：当前不拆分不影响功能，但后续新增算法逻辑时（如小波包分解、自适应阈值），单文件长度可能快速突破 1000 行。

---

### S3 - `projected_stage` 假设完美线性加速，估算可能偏低

- **文件**：`q3_multisource_separation/cli.py`
- **行号**：349-354
- **严重度**：P2
- **类型**：正确性 / 输出一致性
- **问题**：耗时估算函数假设并行效率为 100%（Amdahl 串行部分 0%）。实际中 `pool.map` 的结果反序列化、主进程的 dict 构造、CSV 写入（串行 flush）都会降低有效加速比。

```python
def projected_stage(wall_seconds, new_count, mean_trial, target_count):
    if new_count >= max(20, workers * 2):
        return wall_seconds * target_count / new_count      # 假设完美线性
    if np.isfinite(mean_trial):
        return mean_trial * target_count / workers           # 假设完美线性
```

- **影响**：`q3_runtime.txt` 中 `estimated_full_seconds` 系统性偏低 15-25%。用户可能据此做资源规划（如预计 30 分钟实际需 40 分钟）。
- **建议修复**：
```python
parallel_efficiency = 0.8  # 经验值，可根据 workers 数调整
if np.isfinite(mean_trial):
    return mean_trial * target_count / (workers * parallel_efficiency)
```
或在第二阶段 `mean_trial` 路径中额外添加 15-25% overhead。

---

### S4 - `_run_tasks` 的 `record` closure 使用可变列表，后续改为线程池有竞态风险

- **文件**：`q3_multisource_separation/cli.py`
- **行号**：99-107
- **严重度**：P3
- **类型**：健壮性
- **问题**：`record` 函数闭包捕获了 `pending_write` 可变列表。当前 `pool.map` 是同步提交结果（主进程串行回调），不会出现并发。但若后续改为 `as_completed` 或混用 `ThreadPoolExecutor`，`append_csv_rows(pending_write)` 与 `pending_write.clear()` 之间的非原子操作会丢数据。

```python
pending_write = []
def record(row, count):
    rows.append(row)
    pending_write.append(row)
    if len(pending_write) >= 20 or count == len(tasks):
        append_csv_rows(output_path, pending_write)
        pending_write.clear()  # 竞态：clear 前另一个线程可能已 append
```

- **建议修复**：用 `nonlocal` + 不可变 batch 传递消除竞态：
```python
...
def record(row: dict, count: int) -> None:
    ...
    if len(pending_write) >= 20 or count == len(tasks):
        batch = pending_write
        pending_write = []
        append_csv_rows(output_path, batch)
```
- **影响**：当前无风险，但作为参考留档以供后续扩展。

---

### S5 - `append_csv_rows` schema 校验忽略列顺序

- **文件**：`q3_multisource_separation/outputs.py`
- **行号**：35
- **严重度**：P3
- **类型**：健壮性
- **问题**：用 `set(existing_header) != set(fieldnames)` 比较头部，忽略列顺序差异。如果 column 集合相同但顺序变了（例如 `DictWriter` 按 `fieldnames` 的顺序写入，但不影响键值对解析），校验不会报错，但写入的列顺序会与其后的阅读器的预期不同。
- **建议修复**：
```python
if existing_header != fieldnames:
    if set(existing_header) != set(fieldnames):
        raise ValueError(...)
    # 列顺序变化但列集合相同，使用已有文件的列顺序
    # 可在此处输出 warning
```
- **影响**：极低概率。仅当代码重构了 CSV 字段声明顺序且 `--resume` 追加到旧文件时可能触发。

---

### S6 - `experiments.py` 中有两处未使用的导入

- **文件**：`q3_multisource_separation/experiments.py`
- **行号**：14-17
- **严重度**：P3
- **类型**：代码整洁
- **问题**：`glrt_scan` 和 `multi_harmonic_fit` 被导入但在当前模块中未被直接引用（`fast_spaced_detection` 仅调用 `detect_multitone`，而 `run_simulation_trial` 调用 `fast_spaced_detection`）。
- **建议修复**：移除两处未使用的导入。
- **影响**：仅代码整洁性，不会导致任何运行错误。
